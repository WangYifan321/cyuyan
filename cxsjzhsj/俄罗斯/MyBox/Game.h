...
private:
	...
	int _iLocX, _iLocY;				//方块坐标，Tool类4*4数组左上角位置定义
...
private:
	...
	int  CountNoneZero(int * matrix, int Height, int Width);//计算参数中的一维数组中不为0的元素数
	void AddToolToAux(int *net, int iOffsetX, int iOffsetY, const Tool &_tool);	//将俄罗斯方块标记加入到指定主界面数组中
